...
def pick_group_column(df: pd.DataFrame) -> str:
    """Pick a reasonable group column for grouped CV (pcap/source file)."""
...
def coerce_numeric_dataframe(df: pd.DataFrame, cols):
    """Coerce features to numeric; non-numeric becomes NaN."""
    out = df.copy()
    for c in cols:
...
def make_xgb_classifier(scale_pos_weight=1.0, random_state=42):
    """
    Create XGBClassifier with optional GPU (cuda) fallback to CPU.
    - For XGBoost >= 2.0: tree_method="hist" + device="cuda" uses GPU.
    """
    base_params = dict(
    ...
    # Try GPU first; if it fails, fall back.
    try:
        model = XGBClassifier(**base_params, device="cuda")
        # lightweight "touch" to detect immediate failures in some setups:
        _ = model.get_params()
        return model
    except Exception:
        return XGBClassifier(**base_params)  # CPU fallback
...
def main():
    print("=" * 70)
    print("SPOTIFY vs OTHERS — XGBoost + StratifiedGroupKFold + GridSearchCV")
    print("=" * 70)

    df = pd.read_csv(FILEPATH)

    if "Label" not in df.columns:
        raise ValueError("Expected a 'Label' column (Spotify vs Others).")
    ...
    # Coerce features to numeric (your features should be numeric)
    ...
    print(f"Class counts: Spotify(pos)= {pos}, Others(neg)= {neg}, scale_pos_weight= {spw_all:.3f}")
    ...
    # Grid (keep moderate; grouped CV is heavier)
    param_grid = {
    ...
    grid_search = GridSearchCV(
        ...
        scoring="roc_auc",     # you can change to "f1" if you prefer
        cv=cv_gs,
    ...
    # Extract best model params cleanly
    ...
    # -------------------- Grouped CV with tuned params (OOF) --------------------
    ...
    print(classification_report(y, oof_pred, target_names=["Others", "Spotify"], digits=4, zero_division=0))

    # Confusion matrix plot (OOF)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    cm_path = os.path.join(script_dir, "xgb_groupcv_confusion_matrix.png")
    ...
    plt.plot(fpr, tpr, label=f"OOF ROC (AUC={overall['AUC']:.3f})")
