# spotify_random_forest_grouped_gridsearch.py
...
def grouped_split_80_10_10(df: pd.DataFrame, group_col: str):
    ...
    # 2) VAL = 1 fold out of 9 from train_val
...
def grid_search_rf(X_train, y_train, X_val, y_val, select_by="f1"):
    ...
    print("\n" + "=" * 70)
    print(f"RF GRID SEARCH: {len(grid)} combinations | select_by={select_by.upper()} (VAL)")
    print("=" * 70)
...
def print_final_metrics_and_cm(model, X_test, y_test, title="TEST SET"):
    ...
    out_path = os.path.join(script_dir, "rf_grouped_confusion_matrix.png")
    plot_and_save_confusion_matrix(
        cm,
        class_names=["Others", "Spotify"],
        out_path=out_path,
        title="Random Forest Confusion Matrix (GROUPED TEST SET)",
        cmap="Greens",
    )
...
def main():
    print("=" * 60)
    print("SPOTIFY vs OTHERS - Random Forest (GROUPED 80/10/10 + Grid Search)")
    print("=" * 60)
    ...
    # Optional: sanity check no group leakage
    ...
    # Grid search on Train/Val (grouped splits already)
    best_params, best_score, gs_results = grid_search_rf(
